...
<body>
	...
	<h3>
		<center>Profil użytkownika ${profile.name}
			${profile.lastname}</center>
	</h3>
	...
	<form method="POST">
		<div>
			<p>
				<input type="submit" value="   Zapisz   " /> <input type="button"
					onClick="parent.location='userProfile-list.htm'" value="Wróć" />
			</p>
		...
		<div class="grip">
			<div class="tagBox userView">
				...
				[@spring.bind "profile.id" /] <input type="hidden"
					name="${spring.status.expression}"
					value="${spring.status.value?default(" ")}" /> [@spring.bind
				"profile.user.id" /] <input type="hidden"
					name="${spring.status.expression}"
					value="${spring.status.value?default(" ")}" /> [@spring.bind
				"profile.name" /]
				<p>
				...
				<p>
					<label>Emial:</label> <input type="text" class="inputText"
						disabled="disabled" value="${profile.user.email}" />
				</p>

				[@spring.bind "profile.nick" /]
				<p>
					<label>Nick:</label> <input type="text" class="inputText"
						name="${spring.status.expression}"
						value="${spring.status.value?default(" ")}" />
				</p>
		...
		<div class="grip">
			<div class="tagBox userView">
				...
				<p>
					<label>Strona www: </label> <input type="text" class="inputText"
						name="${spring.status.expression}"
						value="${spring.status.value?default(" ")}" />
				</p>
				...
				<p>
					<label>Numer telefonu: </label> <input type="text"
						class="inputText" name="${spring.status.expression}"
						value="${spring.status.value?default(" ")}" />
				</p>
